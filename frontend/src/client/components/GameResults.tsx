...
import type { GameResults as GameResultsType, PlayerRanking, PlayerStatistics, GameMode } from '../../shared/types/api';
    ...
    <div className={`bg-white/10 backdrop-blur-lg rounded-lg border border-white/20 ${className}`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-white">
                  {getMedalEmoji(currentPlayerRanking.position)} {getPositionSuffix(currentPlayerRanking.position)}
                </div>
                <div className="text-sm text-white/60">Final Position</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-green-400">{currentPlayerRanking.totalScore}</div>
                <div className="text-sm text-white/60">Total Score</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-400">{formatScore(currentPlayerStats.averageScore)}</div>
                <div className="text-sm text-white/60">Average Score</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-400">{formatTime(currentPlayerStats.totalTime)}</div>
      ...
      <div className="p-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-semibold text-white flex items-center gap-2">
                      {ranking.username}
                      {ranking.playerId === currentPlayerId && (
                        <span className="text-xs bg-blue-500/50 text-blue-200 px-2 py-0.5 rounded">You</span>
                      )}
                    </div>
        ...
        {activeView === 'statistics' && (
          <div className="space-y-3">
            <h3 className="text-lg font-semibold text-white mb-4 text-center">Detailed Statistics</h3>
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <div className="text-lg font-bold text-blue-400">{formatScore(stats.averageScore)}</div>
                    ...
                    <div>
                      <div className="text-lg font-bold text-blue-400">{formatTime(stats.totalTime)}</div>
        ...
        {activeView === 'performance' && currentPlayerStats && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-white mb-4 text-center">Your Performance Analysis</h3>
            
            {/* Performance Grade */}
            <div className="bg-gradient-to-r from-blue-600/20 to-blue-500/20 rounded-lg p-6 text-center">
              <div className="text-6xl mb-2">{getPerformanceGrade(currentPlayerStats.averageScore).emoji}</div>
              <div className={`text-4xl font-bold mb-2 ${getPerformanceGrade(currentPlayerStats.averageScore).color}`}>
              ...
              <div className="bg-white/5 rounded-lg p-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-semibold">{formatScore(currentPlayerStats.averageScore)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/70">Total Doors:</span>
                    <span className="text-white font-semibold">{currentPlayerStats.doorsCompleted}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/70">Score per Door:</span>
                    <span className="text-white font-semibold">
                      {formatScore(currentPlayerRanking ? currentPlayerRanking.totalScore / currentPlayerStats.doorsCompleted : 0)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/5 rounded-lg p-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-semibold">{formatTime(currentPlayerStats.totalTime)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/70">Time per Door:</span>
                    <span className="text-white font-semibold">
                      {formatDuration(
                        (parseInt(currentPlayerStats.totalTime) || 0) / currentPlayerStats.doorsCompleted
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-semibold">
                      {currentPlayerRanking ? getPositionSuffix(currentPlayerRanking.position) : 'N/A'}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-blue-500/20 border border-blue-500/30 rounded-lg p-4">
              ...
              <div className="text-blue-100 text-sm space-y-1">
                ...
                {currentPlayerStats.averageScore >= 80 && (
                  <p>• Excellent performance! You're mastering the art of creative problem-solving</p>
                )}
